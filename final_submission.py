...
def main() -> int:
    ...
    for language, label, dataset_name in [
        ...
        dataset_path = Path("data/final_eval") / dataset_name
        print(f"\n{label} DETECTION")
        ...
        try:
            ...
            output_path = Path(save_submission(flagged_ids, TEAM_NAME, language, output_dir="submissions"))
    ...
    for language in ["en", "fr"]:
        filepath = Path("submissions") / f"{TEAM_NAME}.detections.{language}.txt"
        if filepath.exists():
